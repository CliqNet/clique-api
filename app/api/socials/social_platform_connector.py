...
class SocialPlatformConnector:
    ...
    async def generate_oauth_url(self, platform: SocialPlatform, user_id: str, redirect_uri: str) -> str:
    ...
    async def handle_oauth_callback(self, code: str, state: str, redirect_uri: str) -> Dict[str, Any]:
        ...
        user_info = await self._get_platform_user_info(platform, token_data['access_token'])
    ...
    async def _exchange_code_for_tokens(self, platform: SocialPlatform, code: str, redirect_uri: str) -> Dict[str, Any]:
    ...
    async def _get_platform_user_info(self, platform: SocialPlatform, access_token: str) -> Dict[str, Any]:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    await self.db.socialaccount.update(
                        ...
                        data={
                            'accessToken': token_data['access_token'],
                            'refreshToken': token_data.get('refresh_token', account.refreshToken),
                            'expiresAt': expires_at,
                            'lastRefreshed': datetime.utcnow(),
                            'status': ConnectionStatus.CONNECTED.value,
                            'refreshAttempts': 0,
    ...
    async def _handle_refresh_error(self, account_id: str, error: str = None):
        ...
        await self.db.socialaccount.update(
            ...
            data={
                ...
                'lastError': error,
                'status': ConnectionStatus.TOKEN_EXPIRED.value if not error else ConnectionStatus.PLATFORM_ERROR.value,
                'statusUpdatedAt': datetime.utcnow()
            }
        )
    ...
    async def check_rate_limit(self, account_id: str) -> bool:
        ...
        if account.quotaResetAt and now >= account.quotaResetAt:
            await self.db.socialaccount.update(
                ...
                data={
                    'dailyApiCalls': 0,
                    'quotaResetAt': now + timedelta(days=1)
                }
            )
