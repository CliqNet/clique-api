...
async def oauth_callback(
    ...
    if error:
        return RedirectResponse(
            url=f"{frontend_url}/creator/accounts?error={error}",
            status_code=302
        )
    try:
        ...
        frontend_url = os.getenv("FRONTEND_URL", "http://localhost:3000")
        return RedirectResponse(
            url=f"{frontend_url}/creator/accounts?error={str(e)}"
        )
    except Exception as e:
        ...
        "platforms": [
            {
                ...
                "supports_refresh": platform.value in ["FACEBOOK", "YOUTUBE", "LINKEDIN"]
            }
...
async def get_websocket_status(current_user = Depends(get_current_user)):
    ...
    return {
        "user_id": current_user["id"],
        "active_connections": connection_manager.get_user_connections_count(current_user["id"]),
