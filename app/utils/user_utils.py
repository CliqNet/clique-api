...
def check_admin_permission(
    current_user, required_admin_role: Optional[AdminRole] = None
):
    ...
    if required_admin_role and hasattr(current_user, "admin") and current_user.admin:
        ...
        elif required_admin_role == AdminRole.ADMIN and current_user.admin.role in [
            AdminRole.ADMIN,
            AdminRole.SUPER_ADMIN,
        ]:
            return True
        elif required_admin_role == AdminRole.MODERATOR and current_user.admin.role in [
            AdminRole.MODERATOR,
            AdminRole.ADMIN,
            AdminRole.SUPER_ADMIN,
        ]:
...
async def fetch_users_by_type(
    user_type: str, page: int, limit: int
) -> UserListResponse:
    skip = (page - 1) * limit
    ...
    users = await prisma.user.find_many(
        ...
        include={
            ...
            "admin": True,
            # "roles": True,
            "roles": {"include": {"role": True}},  # Include the actual role data
            "sessions": True,
        },
    )
        ...
        if "roles" in user_dict and user_dict["roles"]:
            ...
            for role_data in user_dict["roles"]:
                ...
                if "id" not in role_data:
                    role_data["id"] = (
                        f"{role_data.get('userId', '')}-{role_data.get('roleId', '')}"
                    )
    ...
    return UserListResponse(users=users_with_profiles, pagination=pagination)
