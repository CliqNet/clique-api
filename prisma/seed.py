...
async def create_permissions():
    ...
    for perm_data in permissions_data:
        permission = await prisma.permission.upsert(
            ...
            data={"create": perm_data, "update": perm_data},
        )
...
async def create_roles():
    ...
    roles_data = [
        ...
        },
        {
            ...
            "permissions": [
                ...
                if p.name not in ["create_user", "delete_user"]
            ],
        },
        {
            "name": "super_admin",
    ...
    for role_data in roles_data:
        ...
        await prisma.rolepermission.delete_many(where={"roleId": role.id})

        # Add permissions to role
        for perm_name in role_data["permissions"]:
            if perm_name in perm_dict:
                await prisma.rolepermission.create(
                    data={"roleId": role.id, "permissionId": perm_dict[perm_name]}
                )
...
async def create_admin_users():
    ...
    for admin_data in admin_users:
            ...
            data={
                "create": {
                    **{
                        ...
                        if k not in ["password", "admin_role"]
                    },
        ...
        await prisma.adminprofile.upsert(
            ...
            data={
                "create": {"userId": user.id, "role": admin_data["admin_role"]},
                "update": {"role": admin_data["admin_role"]},
            },
        )
        ...
        if role_id:
            await prisma.userrole.upsert(
                ...
                data={"create": {"userId": user.id, "roleId": role_id}, "update": {}},
            )
...
async def create_sample_creators():
    ...
    for creator_data in creators:
        ...
        await prisma.creatorprofile.upsert(
            where={"userId": user.id},
            data={
                "create": {"userId": user.id, **creator_data["profile"]},
...
async def create_sample_companies():
    ...
    for company_data in companies:
        ...
        await prisma.companyprofile.upsert(
            where={"userId": user.id},
            data={
                "create": {"userId": user.id, **company_data["profile"]},
...
async def create_platform_configs():
    ...
    platform_configs = [
        {
            ...
            "appSecret": os.environ.get(
                "FACEBOOK_APP_SECRET", "YOUR_FACEBOOK_APP_SECRET_NOT_SET"
            ),
            "apiVersion": "v18.0",
    ...
    for config in platform_configs:
        config_entry = await prisma.platformconfig.upsert(
            ...
            data={"create": config, "update": config},
        )
